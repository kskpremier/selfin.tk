...
class  RentsController extends Controller
{
    ...
    public function actionBookings(){
        ...
        $lastUpdate =  $user->myrent_update;
        if ( Yii::$app->user->can('mobile',[]) || Yii::$app->user->can('owner',[]) || Yii::$app->user->can('receptionist',[]) ) {
//            if (($updateTime - $user->updated_at) > MyRentReception::MyRent_USER_UPDATE_INTERVAL) {
...
//            else $rents = $this->synchroService->synchroChangesRentsForUser($user, $lastUpdate, $updateTime, (Yii::$app->user->can('owner',[]))?$user->owner->external_id:null);
            if (($updateTime - $user->myrent_update) > MyRent::MyRent_UPDATE_INTERVAL)
                $rents = $this->synchroService->synchroChangesRentsForUser($user, $lastUpdate, $updateTime, (Yii::$app->user->can('owner',[]))?$user->owner->external_id:null);
            //найти все букинге в базе, независимо от статуса на дату запроса для owner или receptionist
            $bookings = ( Yii::$app->user->can('mobile',[]) )? $this->booking->getBookingsByMobileUser($user->id,$from,$to) :
                $this->booking->getBookingsByOwner($user->owner->external_id,$from,$to);
        }
    ...
    public function serializeRents($bookings)
    {
        ...
        foreach ($bookings as $booking) {
            ...
            $rents[] = [
                ...
                'start_date' => function ($booking) {
                    return date("Y-m-d", strtotime($booking->start_date)) . " " . date("H:i", strtotime($booking->from_time));
                },
                'end_date' => function ($booking) {
                    return date("Y-m-d", strtotime($booking->end_date)) . " " . date("H:i", strtotime($booking->until_time));
                },
                'apartment_id' => 'apartment_id',
                'apartment_name' => function ($booking) {
                    return $booking->apartment->name;
                },
                'external_apartment_id' => function ($booking) {
                    return $booking->apartment->external_id;
                },
                ...
                'price' => function ($booking) {
                    return $booking->price . $booking->label;
                },
                'paid' => function ($booking) {
                    return ($booking->paid == "N") ? false : true;
                },
                'duration' => function ($booking) {
                    $duration = intdiv((strtotime($booking->end_date) - strtotime($booking->start_date)) / 24 / 60, 60);
//                                    return($duration==1)?$duration." day": $duration." days";},
                    return $duration;
                },
                'number_of_guests' => function ($booking) {
//            return ($booking->number_of_tourist==1)?$booking->number_of_tourist." guest":$booking->number_of_tourist." guests";
                    return $booking->number_of_tourist;
                },
                'address' => function ($booking) {
                    return $booking->apartment->adress . ", " . $booking->apartment->city_name;
                },
                'latitude' => function ($booking) {
                    return $booking->apartment->latitude;
                },
                'longitude' => function ($booking) {
                    return $booking->apartment->longitude;
                },
                'note' => function ($booking) {
                    return $booking->note;
                },
                'contact' => function ($booking) {
                    return $booking->author->contact_name;
                },
                'contact_email' => function ($booking) {
                    return $booking->author->contact_email;
                },
                'contact_tel' => function ($booking) {
                    return $booking->author->contact_tel;
                },
                "contact_country" => function ($booking) {
                    return $booking->author->contact_country;
                },
                "contact_country_code" => function ($booking) {
                    return $booking->author->contact_country_code1;
                }
