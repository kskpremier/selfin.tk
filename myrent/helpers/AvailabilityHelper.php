...
//        $byObject = ArrayHelper::index($array, null, 'id');
        ...
        foreach ($array as $object) {
            ...
            $result[$r]['load'] =  (integer) ($occupied/$days);
            ...
            while ($j < count($list)) { //перебираем все букинги
                ...
                if ( $fromUnix <= $current ) { //апартмент занят
                    //присвоить 0 всем ячейкам от $k+1 до $toUnix - крнца букигнга или $to - конца периода
                    $durationOfRent = (integer)((min($toUnix, $to) - $current) / $day);//длина букинга в днях
                    $occupied = $occupied + $durationOfRent;
                    for ($i = 0; $i < $durationOfRent; $i++) {

                        $result[$r]['data'][$i + $k + 1] = -$i;
                    }
                    $k = $k + $durationOfRent;
                    $current = min($toUnix, $to);
...
//                    $result[$r]['empty'] = ($empty)? $occupied. " from ". $empty. " days" : "0 from ". $days. " days";
            ...
            $result[$r]['empty'] =  $occupied. " from ". $days;
            $result[$r]['load'] = (integer) (100* $occupied/$days);
            $r++; //следующий объект
            }
        return $result;
    }
