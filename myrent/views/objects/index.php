...
//формируем некоторые заголовки
    ...
    $data = (count($data))? $data :  ArrayHelper::getColumn($dataProvider->query->select('objects.name')->all(),'name');
            ...
            <tr>
                <th rowspan="1" style="width:<?=$widthName?>%; vertical-align: top; text-align: center; ">
                    <?php  echo $form->field($searchModel, 'name')->widget(Typeahead ::classname(), [
                                        ...
                                        'dataset' => [
                                            [
                                                'local' => (count($data)>0)?$data : ["'<div class=\"text-danger\" style=\"padding:0 8px\">Unable to find repositories for selected query.</div>'"],
                                                'limit' => 10
                                            ]
                                        ]
                                    ])
